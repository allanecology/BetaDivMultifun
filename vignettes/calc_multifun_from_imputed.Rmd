...
- nitrate oxidising group : NS, NB, nifH, nitrate.production.availability (inverted DEA)
...
#TODO : take out as soon as seed depletion is imputed
usedforBetadivMultifun <- usedforBetadivMultifun[!usedforBetadivMultifun == "seed.depletion"]
...
corrplot::corrplot(M,type="lower",addCoef.col = "black",method="color",diag=T, tl.srt=1, tl.col="black", mar=c(0,0,0,0), number.cex=0.6, order = "hclust")
...
t <- as.matrix(EFbeta)
t[!lower.tri(t)] <- NA
t <- reshape2::melt(t, value.name = "EFbeta")
t <- t[!is.na(t[, 3]), ]
...
plotEFbeta <- reshape2::melt(EFmaster[, c("Var1", "Var2", "EFbeta", "EFturnover", "EFnestedness")], id = c("Var1", "Var2"))
p <- ggplot2::ggplot(data = plotEFbeta, aes(x = variable, y = value)) +
  ...
  labs(title="Threshold-based dissimiliarity of Ecosystem Functions", xlab = "") + 
