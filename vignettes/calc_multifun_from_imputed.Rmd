...
```{r}
#TODO : take out some clusters? (Pete's suggestion at meeting)
...
```{r}
# imputed_grlfuns is renamed because of old code
#TODO : rename all fungrl with imputed grlfuns in new code

fungrl <- data.table::copy(imputed_grlfuns)

# STANDARDISATION
include <- names(fungrl)[!names(fungrl) %in% "Plotn"]
fungrl[, (include) := lapply(.SD, scale01),.SDcols=mysubset]
...
pca_fungrl <- stats::prcomp(pc_fungrl, scale=F) 
   # plot pca
plot(pca_fungrl)
biplot(pca_fungrl)
plot(ecdf(pca_fungrl$sdev))
```
**axis selection** : 
- first axis for sure
- 2nd to 4th : explain around 35%
- 5th to 7th : explain around 55%
- 8th to 10th : explain around 80%
There are groups of three axis which have about the same amount of variance explained. 
```{r}

#TODO : HERE! chose the PCA axis to include and the weighting we will apply to it (equal?)


# 
# pc_fungrl <- vegan::scores(pca_fungrl, display="sites")
# rownames(pc_fungrl) <- fungrl$Plotn


# saveRDS(..., "EFdistance.rds")
```
