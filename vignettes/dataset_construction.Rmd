...
data.table::setnames(synthesis, old="DEA", new="DEA_with_C2H2")
```
...
forage.quality <- data.table::setnames(forage.quality[,c("Plot_name", "forage.quality")], old=c("Plot_name", "forage.quality"), new= c("Plot", "forage.quality"))
```
...
```{r}
P.loss2011 <- data.table::fread(info_data$path_unix[grep("P.loss2011", info_data$ColumnName)], header=T, sep='\t')
```
...
data.table::setnames(OlsenPi2014, old=c("Plot_ID", "OlsenPi"), new=c("Plot", "OlsenPi2014"))
OlsenPi2014 <- merge(PlotExplo, OlsenPi2014, by="Plot")
...
data.table::setnames(microbial_enzyme_activities, old=c("EP_Plot_ID"), new="Plot")
...
CompareToReferenceDataset(microbial_enzyme_activities, synthesis, name="DEA_with_C2H2", sortcol = "Plot")
