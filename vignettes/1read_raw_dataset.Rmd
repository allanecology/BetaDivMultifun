...
# Pathogen infection
update : is created from assembled data, available locally only.
...
npla <- data.table::fread("/run/user/1001/gvfs/smb-share:server=ipssmonstera.unibe.ch,share=planteco/DATA/BE_Synthesis/BetaDivMultifun/raw_data/24247_Vegetation Records for Grassland EPs, 2008 - 2018_1.1.1/24247.txt", header=T)
...
Poa[, Species := "Poa"]
pla2 <- rbind(npla[-grep("Poa", Species)], Poa)

#TODO I guess pc is not needed here...
pc2 <- pc[-grep("Poa", pc$Species),] # correct this in the species classification as well
pc2$Species <- as.character(pc2$Species)
p <- c("Poa","G")
pc2 <- rbind(p, pc2)
...
### total infection per plant species per plot
# aggregate different pathogens per plant and plot (for all plant species which are affected by more than one pathogen)
patt <- cbind(pat, tot.infect)
# aggregate
p.infect <- aggregate(tot.infect~Plot+host, patt, sum)
```
...
sum(npla[Species == "Carex_hirta",Cover])
sum(npla[Species == "Cardamine_hirsuta",Cover])
...
sum(npla[Species == "Rumex_acetosa",Cover])
sum(npla[Species == "Rumex_acetosella",Cover])
...
test <- merge(p.infect, pla2, by=c("Plot", "Species"), all.x=T)
test <- data.table::as.data.table(test)
###### multiply total infection by relative cover of plant species, assume that a missing species is not infected.
test[, p.inf := tot.infect * Cover]
pathogen.infection <- aggregate(p.inf.perplant~Plot, test, sum)
```
