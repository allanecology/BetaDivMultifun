...
# raw dataset
Generate a dataset which stores all raw data. Will be assembled with the script `assembled_dataset_construction.R`. Datasets that can not be easily included as raw datasets into `raw_functions` are assembled (calculations are done) in `assembled_dataset_construction.R` and then integrated into raw_functions as well (and assembled functions variable, of corse).
```{r}
# subset for raw dataset construction
raw_functions <- data.table::copy(PlotExplo)
# generate list for datasets that will be constructed in assembled_dataset_construction.R script directly.
...
raw_functions <- merge(raw_functions, Biomass2011, by="Plot")
rm(Biomass2011)
...
raw_functions <- merge(raw_functions, Biomass2012, by="Plot")
rm(Biomass2012)
...
raw_functions <- merge(raw_functions, Biomass2013, by="Plot"); rm(Biomass2013)
...
raw_functions <- merge(raw_functions, Biomass2016, by="Plot"); rm(Biomass2016)
...
raw_functions <- merge(raw_functions, Pmic.2014, by="Plot") ; rm(Pmic.2014)
```
...
```{r}
herb <- data.table::fread(info_data$path_unix[grep("herbivory.2017", info_data$ColumnName)], header=T, sep='\t')
data.table::setnames(herb, old=c("plotid_withzero", "plotid"), new=c("Plotn", "Plot"))

# 2017
herb2017 <- herb[which(herb$years == "2017")]
data.table::setorder(herb2017, Plotn)
list_raw_functions[["Herbivory.2017"]] <- herb2017
rm(herb2017)

# 2018
herb2018 <- herb[which(herb$years == "2018")]
data.table::setorder(herb2018, Plotn)
list_raw_functions[["Herbivory.2018"]] <- herb2018
rm(herb2018); rm(herb)
```
...
raw_functions <- merge(raw_functions, P_loss2011, by="Plot"); rm(P_loss2011)
```
...
raw_functions <- merge(raw_functions, P_loss2015, by="Plot"); rm(P_loss2015)
```
...
raw_functions <- merge(raw_functions, OlsenPi2014, by="Plotn"); rm(OlsenPi2014)
```
...
raw_functions <- merge(raw_functions, microbial_enzyme_activities, by="Plot"); rm(microbial_enzyme_activities)
```
...
raw_functions <- merge(raw_functions, Potential.nitrification, by="Plot"); rm(Potential.nitrification)
```
...
raw_functions <- merge(raw_functions, Nmic.2011, by="Plot"); rm(Nmic.2011)
```
...
raw_functions <- merge(raw_functions, NH4.2014, by="Plot"); rm(NH4.2014)
```
...
raw_functions <- merge(raw_functions, mAMFhyphae, by="Plot", all.x = T, all.y = T); rm(mAMFhyphae)
```
...
# raw_functions <- merge(raw_functions, Gtot, by="Plot")
```
...
# amoA_AOA and AOB
```{r, eval=F}
data.table::fread("/run/user/1001/gvfs/smb-share:server=ipssmonstera.unibe.ch,share=planteco/DATA/BE_Synthesis/BetaDivMultifun/raw_data/21547_Nitrifiers Abundances at Grassland EPs_1.6.8/21547.txt")
```
