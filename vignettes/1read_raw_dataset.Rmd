...
info_data <- info_data[grep("/raw_data/", info_data$path_unix),c("ColumnName", "path_unix")]
```
...
# read in 2014 data
nutrients2014 <- data.table::fread(info_data$path_unix[grep("Nshoot.2014", info_data$ColumnName)])
nutrients2014 <- nutrients2014[Year == "2014", c("Plot_ID", "N", "P")]
data.table::setnames(nutrients2014, old = c("Plot_ID", "N", "P"), new=c("Plot", "Nshoot2014percent", "Pshoot2014percent"))
...
Nmic.2014 <- data.table::fread(info_data$path_unix[grep("Nmic.2014", info_data$ColumnName)])
...
```{r, eval=F}
# 2016
amo <- data.table::fread(info_data$path_unix[grep("amoA_AOB.2016", info_data$ColumnName)])
data.table::setnames(amo, old=c("Plot_ID"), new=c("Plot"))
raw_functions <- merge(raw_functions, amo[,c("Plot","amoA_AOA.2016", "amoA_AOB.2016")], by="Plot", all = T); rm(amo)

# 2011
nit2011 <- data.table::fread(info_data$path_unix[grep("nifH", info_data$ColumnName)])
data.table::setnames(nit2011, old=c("PlotID", "amoA_AOA", "amoA_AOB"), new=c("Plot", "amoA_AOA.2011", "amoA_AOB.2011"))
raw_functions <- merge(raw_functions, nit2011[, c("Plot", "amoA_AOA.2011", "amoA_AOB.2011", "nifH")], by="Plot", all=T)
```
