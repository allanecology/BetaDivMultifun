...
- "BE.pathogen.abs.txt" : contains abbreviations for each species
...
# read in 3 datasets with the following names
datasetnames <- c("BE.grassland.flower.numbers.txt", "Flower_sizes.txt", "Trap.nests.txt")
paths <- paste(path, datasetnames, sep="")
```

**flower cover**
```{r}
fl.num <- read.table(paths[1], header = T)
fl.size <- read.table(paths[2], header = T)
...
#TODO write.table(as.matrix(cov.plot), "flower.cover.plot.total.txt")
```

**Trap nesting**
```{r}
traps <- read.table(paths[3], header = T)
# get summary of data
...
#TODO write.table(traps2,"Trap.nest.Parasitoids.txt", row.names =F)
```
...
##################### not possible to go further because this file is missing. Without this file, species names are not matching!
# pc <- read.table("Plants 2011 species classification.txt", header =T)
# #TODO : the file path is not given for the above file. I could not find it.
# pc2 <- pc[-grep("Poa", pc$Species),]
