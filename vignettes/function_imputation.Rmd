...
source("clean_and_load_soil_covariates.Rmd")
```
...
It also relies on a soil information dataset, which is prepared by `clean_and_load_soil_covariates.Rmd`.

Requires the plot set defined by diversity dataset with the set of included plots for analysis.
...
New : values with too many missings are not taken out any more. Instead, the imputation is done and the variables are taken out in case their imputation error is too high.
```{r, eval=F}
...
```{r}
# take out colnames of glsoil, except "Plotn"
Y[, (colnames(glsoil)[!colnames(glsoil) %in% c("Plotn", "Soil.type")]) := NULL]
