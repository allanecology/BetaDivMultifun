...
for(m in model_names[model_class %in% "multifun" & lui == "LUI", modelname]){
  #TODO implement for single functions
  m <- model_names[model_class %in% "multifun" & lui == "LUI", modelname][10]
  
  model_names_selection <- model_names[which(model_names$modelname == m), ]
  # run analysis and create results
  ksource("vignettes/prepare_and_run_GDM.Rmd") 
  ksource("vignettes/check_GDM_input.Rmd") # produce nice plots/ results about correlations among input variables
  ksource("vignettes/plot_modelwise_GDM.Rmd") # produce bar and line plots of the current model
  #TODO HERE make run for all m! and use output data to produce overview over thresholds
}
```
...
ksource("plot_unique_GDM.Rmd") # plot single EF models
ksource("summarise_GDM_results.Rmd") # create summary tables of all model results
```
