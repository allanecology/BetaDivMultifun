...
# autotrophs
```{r}
subset <- masterdiversity[, .(autotroph.beta.sim, autotroph.beta.sne, autotroph.alpha, autotroph.alpha.2)]

plot_nestedness <- ggplot(subset, aes(y = autotroph.alpha, x = autotroph.alpha.2)) +
  geom_point(aes(colour = autotroph.beta.sne, size = 2)) +
  # scale_colour_gradient(low = "blue", high = "red")
  scale_colour_viridis_c()

plot_turnover <- ggplot(subset, aes(y = autotroph.alpha, x = autotroph.alpha.2)) +
  geom_point(aes(colour = autotroph.beta.sim, size = 2)) +
  # scale_colour_gradient(low = "yellow", high = "violet")
  scale_colour_viridis_c()

plot_alpha_autotroph <- plot_grid(plot_turnover, plot_nestedness, labels = c("to", "nes"))
```
...
for(t in trlevels){
  ...
  plot_turnover <- ggplot(subset, aes(y = get(namesref[["alpha"]]), x = get(namesref[["alpha2"]]))) +
    geom_point(aes(colour = get(namesref[["beta.sim"]])), size = 7) +
    scale_colour_viridis_c() +
  ...
  plot_nestedness <- ggplot(subset, aes(y = get(namesref[["alpha"]]), x = get(namesref[["alpha2"]]))) +
    geom_point(aes(colour = get(namesref[["beta.sne"]])), size = 7) +
    scale_colour_viridis_c() +
  ...
  p <- plot_grid(plot_turnover, plot_nestedness, labels = c("to", "nes"))
  assign(plotname, p)
