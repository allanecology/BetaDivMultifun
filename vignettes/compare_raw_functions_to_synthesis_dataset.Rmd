...
```
Run after running `assembled_dataset_construction.R`. There, the datasets which can not be included in the raw_functions variable are assembled and added to `raw_functions` to be able to compare to `synthesis`.
```{r}
CompareToReferenceDataset(raw_functions, synthesis, name ="herbivory", sortcol = "Plot")
```


## within range
### Parasitoid traps
see calculation in raw_dataset
```{r}
test <- merge(synthesis[,c("Plot", "Parasitoid.traps")], raw_functions[,c("Plot", "Parasitoid.traps")], by="Plot")
max(test$Parasitoid.traps.x - test$Parasitoid.traps.y, na.rm=T)
CompareToReferenceDataset(raw_functions, synthesis, name ="Parasitoid.traps", sortcol = "Plot")
```

### Groundwater recharge

```{r}
test <- merge(synthesis[, c("Plot", "Groundwater.recharge")], raw_functions[, c("Plot", "Groundwater.recharge")], by="Plot")
max(test$Groundwater.recharge.x - test$Groundwater.recharge.y)
```
...
```{r}
t <- merge(synthesis[,c("Plot", "Litter.decomposition")], Litter.decomposition, by="Plot")
max(t$Litter.decomposition.x - t$Litter.decomposition.y)
```

### Nshoot and Pshoot 2011
```{r}
t <- merge(synthesis[,c("Plot", "Nshoot.2011", "Pshoot.2011")], raw_functions[,c("Plot", "Nshoot.2011", "Pshoot.2011")], by="Plot")
max(t$Nshoot.2011.x - t$Nshoot.2011.y, na.rm=T)
max(t$Pshoot.2011.x - t$Pshoot.2011.y, na.rm=T)
```
...
```{r, eval=F}
test <- merge(synthesis[, c("Plot", "Total.pollinators")], raw_functions, by="Plot")
test[which(test$Total.pollinators.x - test$Total.pollinators.y > 1),]
# change number from synthesis dataset
synthesis[Plot == "AEG24", ]$Total.pollinators <- 339
#TODO : still returns FALSE; but I controlled all by hand and it seems to be correct. (equal number of NAs, all values are the same.)
...
```{r}
merge(Biomass2009, synthesis[,c("Plot", "Biomass2009")], by="Plot", all=T)
```


### forage quality
Note : **multiply with biomass from the given year!**
```{r}
