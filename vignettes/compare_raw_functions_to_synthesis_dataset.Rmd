...
```
Run after running `2calc_raw_dataset.R`. There, the datasets which can not be included in the raw_functions variable are assembled and added to `raw_functions` to be able to compare to `synthesis`.
```{r}
...
```{r, eval=F}
Biomass2009 <- raw_functions[, c("Plot", "Biomass2009")]
deleteme <- merge(Biomass2009, synthesis[,c("Plot", "Biomass2009")], by="Plot", all=T)
```
