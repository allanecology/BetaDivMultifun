...
# Procedure
1. **calculate Threshold averages** : with script `cluster_scripts_documentation.Rmd` in order to prodcue the input datasets for this script
2. Quality check of step 1
  1. calculate threshold averages for maxspline
  2. calculate threshold averages for whole ispline
...
Aim of this section : assess the quality of the uncertainty calculation done in `cluster_scripts_documentation.Rmd`. Are the datasets complete? Do the values make sense?

```{r}
# Quality check of cluster stuff
par(mfrow = c(1, 2))
put01 <- readRDS("~/Desktop/BEF_cluster/gdm_EFturnover_0.8_LUI_uncertainty.Rds")
...
Calculate weighted average over all models. The GDM models are not all of the same quality. They are weighted based on their XXX
...
Create a plot with average effect sizes across the 3 main models : EFturnover_WEIGHTED_mean, EFnestedness_WEIGHTED_mean, EFdistance.

Only biotic effects, because we add turnover and nestedness together, and a comparison with LUI would not be fair (2 effects vs. 1 effect).

Average effect size : Depreciated, because it is almost exclusively driven by EFdist model, because it is on a different scale.
- either scale and average (not done)
- or take average rank (done)
```{r}
