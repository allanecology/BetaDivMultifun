...
1.  **calculate Threshold averages** : with script
    `cluster_scripts_documentation.Rmd` in order to prodcue the input
    datasets for this script
2.  Quality check of step 1
3.  calculate threshold averages for maxspline
4.  calculate threshold averages for whole ispline
...
Aim of this section : assess the quality of the uncertainty calculation
...
#
#TODO does the package has a problem here?
# the first plot just looks correct, and the second plot not.
plot(put01$LUI_fullModel_X, put01$LUI_fullModel_Y, type = "l", ylim = range(put01$LUI_fullModel_Y) + c(-0.02, 0.05))
polygon(c(put01$LUI_fullModel_X, rev(put01$LUI_fullModel_X)), c(put01$LUI_minusSD_Y, rev(put01$LUI_plusSD_Y)), col = "gray", border = NA)
# lines(put01$LUI_fullModel_X, put01$LUI_minusSD_Y)
# lines(put01$LUI_fullModel_X, put01$LUI_plusSD_Y)
lines(put01$LUI_fullModel_X, put01$LUI_fullModel_Y)
#
plot(put01$LUI_fullModel_X, put01$LUI_fullModel_Y, type = "l", ylim = range(put01$LUI_fullModel_Y) + c(-0.02, 0.05))
polygon(c(put01$LUI_minusSD_X, rev(put01$LUI_plusSD_X)), c(put01$LUI_minusSD_Y, rev(put01$LUI_plusSD_Y)), col = "gray", border = NA)
lines(put01$LUI_fullModel_X, put01$LUI_fullModel_Y)
```
...
ispline_uncertainty_all_thresholds <- lapply(seq_along(ispline_uncertainty_all_thresholds), 
                function(y, n, i) {y[[i]]$model_name <- n[i]; y[[i]]$value_ID <- seq(1, 200); return(y[[i]])},
       y = ispline_uncertainty_all_thresholds,
       n = names(ispline_uncertainty_all_thresholds))
# pile all datasets on top of each other
ispline_uncertainty_all_thresholds <- rbindlist(ispline_uncertainty_all_thresholds)
# filter out the relevant columns : exclude x uncertainties
relevant_column_names <- c("model_name", "value_ID", grep("_fullModel_X|_minusSD_Y|fullModel_Y|_plusSD_Y", names(ispline_uncertainty_all_thresholds), value = T))
...
ggplot(ispline_uncertainty_all_thresholds[model_name == "gdm_EFturnover_0.2", .(autotroph.beta.sne_fullModel_X, autotroph.beta.sne_fullModel_Y)], aes(x = autotroph.beta.sne_fullModel_X, autotroph.beta.sne_fullModel_Y)) +
  geom_point()
...
ggplot(ispline_uncertainty_all_thresholds, aes(x = autotroph.beta.sne_fullModel_X, autotroph.beta.sne_fullModel_Y, col = model_name)) +
  geom_point()
ggplot(ispline_uncertainty_all_thresholds, aes(x = autotroph.beta.sim_fullModel_X, autotroph.beta.sim_fullModel_Y, col = model_name)) +
  geom_point()
...
Calculate $w$ for each value of X for each predictor for each model individuall. In other words, calculate how wide the confidence interval is at a given place in X for a given predictor and model.
Speciality : All models have the same X values, which are identified with value_ID.
...
#        - include the missing two models turnover 0.8 and 0.9 --> calc by hand!
...
# # ispline_uncertainty_all_thresholds[grep("nestedness", ispline_uncertainty_all_thresholds$model_name), 
# #                                    model_class := "nestedness"]
```
