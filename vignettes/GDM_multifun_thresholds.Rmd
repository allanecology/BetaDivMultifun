...
restab <- data.table(aggregate(restab, weighted_means ~ legendnames, FUN = mean))

# sort by effect size
setorder(restab, cols = "weighted_means")

# plot
restab <- merge(restab, unique(nicenames[, .(legendnames, color)]), by = "legendnames", all.x = T)
restab[, legendnames := factor(legendnames, levels = restab$legendnames)]
pw <- ggplot(data = restab, aes(x = legendnames, y = weighted_means, fill = color)) +
  geom_bar(stat = "identity", color = "black") + 
  coord_flip() +
  scale_fill_identity() +
  ylab("averages across main models") + xlab("")
...
plot_grid(p, pw)
#TODO HERE : discuuss: makes a huge difference if weighted by sd or not!
# Do I even have to weight again? Go through that carefully.
```
