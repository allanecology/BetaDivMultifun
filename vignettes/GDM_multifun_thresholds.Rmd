...
for(mtype in c("nestedness", "turnover")){
  for(pred in unique(ispline_uncertainty_all_thresholds$names)){
    p <- create_gdm_lineplot_predictorwise_thresholds(
      data = ispline_uncertainty_all_thresholds[Model_type == mtype & names == pred, ], #TODO here isch komisch
      ribbon = T,
      legend = T,
      plottitle = paste(pred, mtype),
      ymax = min(1, max(ispline_uncertainty_all_thresholds[Model_type == mtype & names == pred, plusSD_Y], na.rm = T), na.rm = T))
    plot(p)
  }
}
dev.off()
