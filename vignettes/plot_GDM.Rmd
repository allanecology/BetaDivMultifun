...
EFmean <- data.table(aggregate(value ~ component + nicenames, restab1, mean))
...
# save dataset
saveRDS(restab, file = "vignettes/out/plot_GDM_results_EFthresholds_mean_sd.rds")
```
plot
```{r}
restab <- readRDS(paste(pathtodata, "/analysis/output_datasets/plot_GDM_results_EFthresholds_mean_sd.rds", sep = ""))
...
ggsave(filename = paste(pathtoout, "/plot_GDM_thresholds_heatmap_mean_sd.pdf", sep = ""), plot = heatmap, device=cairo_pdf,
       width = 11.96, height = 8.27, units = "in", dpi = 900)
```

### barplot average effects over all thresholds
```{r}
restab_efdist # from EFdistLUI
restab <- readRDS(paste(pathtodata, "/analysis/output_datasets/plot_GDM_results_EFthresholds_mean_sd.rds", sep = ""))
include <- c("nicenames", "EFturnover_mean", "EFturnover_sd", "EFnestedness_mean", "EFnestedness_sd")
restab <- restab[, ..include]
restab <- merge(nicenames, restab, by = "nicenames")
...
# add sd
###########
p <- p + geom_errorbar(aes(ymin = maxsplines - sd/2, ymax = maxsplines + sd/2), 
                  linetype = "solid") +
  ylim(-0.01, 0.45)
...
#TODO okay to divide sd by two?
#TODO HERE NO! better use standard error!
