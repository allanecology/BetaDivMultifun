...
```{r}
p<-ggplot(data = restab[type == "bio"], aes(x=nicenames, y=onlysign, fill = names)) +
  geom_bar(stat="identity", color = "black", linetype = rep(c("solid", "dashed"), 16)) + 
  coord_flip() + 
  scale_fill_manual(values = varcols) +
  ylim(0, 0.4) + 
...
q <- ggplot(data = restab[type == "abio"], aes(x=nicenames, y=onlysign, fill = names)) +
...
plot_grid(p, q, labels = c('A', 'B'), label_size = 12, nrow = 2, rel_heights = c(0.8, 0.2), align = "v")
```
