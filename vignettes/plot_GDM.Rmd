...
#TODO : chose either LUI OR components
if(luiinput == "lui"){
  # LUI
  multipanel_with_overview_bars <- plot_grid(p, b, q, ov1, ov2, labels = c('A', '', 'B', 'C', ''), label_size = 12, nrow = 5,
                                             rel_heights = c(0.23, 0.37, 0.14, 0.1, 0.1), align = "v") # 2 summary bars
...
```{r}
#TODO HERE : leider habe ich geosphere_distgeo in den modellen drin --> ev. alle modelle nochmal neu?
# schauen wo es drin ist und nochmal drüber!
...
for(i in mod_names[-1]){
  ...
  # append to restab
  rbindlist(list(restab, restab_newline), use.names = T)
}

# append to restab
rbindlist(restab, restab_newline)
