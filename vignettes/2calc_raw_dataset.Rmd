...
unscaled <- ggplot(melt(Gtot), aes(factor(variable), value, fill = variable)) + geom_violin() + scale_color_brewer(palette="Set1")
...
scaled <- ggplot(melt(scd), aes(factor(variable), value, fill=variable)) + geom_violin() + scale_color_brewer(palette="Set1")

# comparing means
means <- merge(scd, Gtot, by="Plot")
means <- means[, c("Plot", "mean_of_years", "unscaled_mean")]
means[, c("mean_of_years", "unscaled_mean") := apply(means[,-1], 2, scale)]

scmeans <- ggplot(melt(means), aes(factor(variable), value, fill=variable)) + geom_violin() + scale_color_brewer(palette="Set1")

plot_grid(unscaled, scaled, scmeans, labels = c("A","B", "C"))
```
