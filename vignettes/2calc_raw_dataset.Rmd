...
assembled_functions <- merge(assembled_functions, Root.biomass, by="Plot", all=T)
```

# Groundwater recharge
```{r}
Gtot <- list_raw_functions$Groundwater.recharge
assembled_functions[, Groundwater.recharge := rowMeans(Gtot[,c(paste("Groundwater.recharge", seq(2010,2016), sep=""))])]
```
...
# add to assembled_functions
assembled_functions <- merge(assembled_functions, assemblednutrients[,c("Plot", "Nshoot.2010", "Pshoot.2010")], by="Plot")
...
# add to assembled_functions
assembled_functions <- merge(assembled_functions, assemblednutrients[,c("Plot", "Nshoot.2011", "Pshoot.2011")], by="Plot")

# 2012
assemblednutrients <- merge(Biomass2012, nutrients, by="Plot", all=T)
...
assembled_functions <- merge(assembled_functions, assemblednutrients[,c("Plot", "Nshoot.2012", "Pshoot.2012")], by = "Plot", all = T)

# 2013
assemblednutrients <- merge(Biomass2013, nutrients, by="Plot", all=T)
...
assembled_functions <- merge(assembled_functions, assemblednutrients[,c("Plot", "Nshoot.2013", "Pshoot.2013")], by = "Plot", all = T)
...
2 samples per plot in 145 cases, 1 sample for :  "AEG6"  "HEG16" "HEG43" "SEG19" "SEG33" and 3 samples for "SEG45". 2 Columns are included in the assembled_functions dataset, 1 for each sample.
...
assembled_functions <- merge(assembled_functions, Herbivory.2013, by="Plot", all.x=T)
```
...
assembled_functions <- merge(assembled_functions, Litter.decomposition, by="Plot", all.x = T)
```
...
assembled_functions <- merge(assembled_functions, pt, by="Plot", all.x = T)
```
...
assembled_functions <- merge(assembled_functions, Total.pollinators, by="Plot", all=T)
```
...
cat("Dear user, \n
    ...
    '_assembled_functions_dataset.csv' \n
    example : 'december2018' would give 'december2018__assembled_functions_dataset.csv'")
prefix <- readline(prompt="Write now : ")
write.csv(assembled_functions, file=paste("~/Desktop/", prefix, "_assembled_functions_dataset.csv", sep=""))
```
