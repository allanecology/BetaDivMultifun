...
# invert the function, such that high values represent low herbivory
Herbivory.2013$herbivory.2013 <- max(Herbivory.2013$herbivory.2013, na.rm=T) - Herbivory.2013$herbivory.2013
...
## 2017 2018
Invert values, such that high values represent low herbivory.
```{r}
raw_functions$herbivory.20172018 <- max(raw_functions$herbivory.20172018, na.rm=T) - raw_functions$herbivory.20172018
```


## Combining 2013 with 2017/2018?
TODO : can we combine them?

The range differs.
```{r}
hist(scale(raw_functions$herbivory.2013))
hist(scale(raw_functions$herbivory.20172018))
plot(scale(raw_functions$herbivory.2013), scale(raw_functions$herbivory.20172018))
hbv <- scale(raw_functions[, c("herbivory.2013", "herbivory.20172018")])

ggplot2::ggplot(data.table::melt(hbv), ggplot2::aes(factor(Var2), value, fill = Var2)) + ggplot2::geom_violin() + 
  ggplot2::scale_color_brewer(palette="Set1") + ggplot2::geom_jitter(shape=16, position=ggplot2::position_jitter(0.4)) # ggplot2::geom_dotplot(binaxis='y', stackdir='center', dotsize=0.4, fill="black")
```
...
# invert the function, such that high values represent low infection
pathogen.infection[, pathogen.infection := (max(pathogen.infection, na.rm=T) - pathogen.infection)]
