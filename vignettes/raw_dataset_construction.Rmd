...
```{r}
#TODO how to name this? make extra raw dataset ad go to assembled datast constr. to do
pathogen.infection1 <- data.table::fread(info_data$path_unix[grep("pathogen.infection", info_data$ColumnName)])
pathogen.infection1 <- pathogen.infection1[, c("EP_plot_ID", "mean_prevalence","mean_severity")]
data.table::setnames(pathogen.infection1, old=c("EP_plot_ID", "mean_prevalence","mean_severity"), new=c("Plot", "mean_prevalence","mean_severity"))
raw_functions <- merge(raw_functions, pathogen.infection1, by="Plot", all.x = T)

test <- merge(raw_functions, synthesis[,c("Plot", "pathogen.infection")])
```
