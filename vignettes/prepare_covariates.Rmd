...
```{r}
#TODO : add Region to the soil pca?
```
...
glsoil[, (mysubset) := lapply(.SD, scale01),.SDcols=mysubset]
#TODO : revise this scaling method - should I better substract the mean and divide by sd?
# in the latter case, I need to take out the binary variables.
```
...
# run the pca
pca_from_glsoil <- prcomp(pc_glsoil[, !(names(pc_glsoil) %in% "Soil.type")], scale = FALSE)
...
# save as RDS
saveRDS(edis_glsoil, file="../data/original/soil_covariate.RDS")
...
# glsoil is used for gap-filling and therefore not deleted here.
```
