...
bring to range [0,1]
```{r}
scale(glsoil[,-1], scale = apply(glsoil[,-1], 2, max))

#TODO here! not sure how to scale ... bring all to same range, right? why is categorical stuff so strange after scaling?
# just use the function I wrote below?

# test <- apply(glsoil[,-1], 2, function(x) scale(x, center=F, max(x)))
```
...
mysubset <- names(glsoil)[!names(glsoil) %in% "Plotn"]
