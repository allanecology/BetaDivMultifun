...
for(i in 1:50){
  ...
  impvals[[i]] <- as.matrix(current)
}

#TODO : probably after here the error occurred!


# extract imputed values and calculate mean
# get all imputed values as vectors in a list of 50 vectors and transform to matrix
impvalsvec <- lapply(impvals, function(x) as.numeric(x[naloc]))
impvalsmat <- data.frame(matrix(unlist(impvalsvec), ncol=length(impvalsvec), byrow=T))
# calculate means and plot
impvalsmat[, "means"] <- rowMeans(impvalsmat)
```
