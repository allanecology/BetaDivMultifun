...
# - ev. Diversity dataset cleaning skript machen? Separieren von calc_betadiversity...?

#TODO INS REINE SCHREIBEN : 
# - sauber alle unbenutzten trophic levels entfernen
# - ev. alle excluded trophic levels nach hinten befördern
...
spdiv <- spdiv[!Trophic_level %in% c("unknown.protist", "unknown.soilfungi", "unknown.acari", "herbivore.bird", "protist.parasite.nonplant", "mainlybacterivore.protist", "decomposer.acari")]
## herbivore.bird, former vertebrate herbivore : only 104 plots

# take out trophic levels which were excluded during this analysis
# OPTION : comment out to reproduce reason of exclusion
exclude <- c("pollinator.arthropod", "decomposer.arthropod", "decomposer.myriapod", "decomposer.soilfungi", "omnivore.arthropod", "omnivore.protist", "omnivore")
#TODO : go through trophic levels and find out which ones to include and which not. Sind da wirklich so viele neue namen, oder sind sie nur umbenennnt etc.?
...
spdiv[Trophic_level == "omnivore.ant", Trophic_level := "omnivore"]
spdiv[Trophic_level == "omnivore.snail", Trophic_level := "omnivore"]
spdiv[Trophic_level == "mainlybacterivore.protist", Trophic_level := "bacterivore.protist"]
# unique(spdiv$Trophic_level)
```

show years
```{r, eval=F}
#TODO : move this downwards, after selecting trophic levels
unique(spdiv[, .(Trophic_level, Year)])
```

```{r, eval=F}
length(unique(spdiv$Species)) # 191696 species in the dataset (incl. OTU)
```
Get overview of amount of species per trophic level
```{r, eval=F}
#TODO : also move downwards
...
# exclude trophic levels which were excluded due to reasons below
#  (found out during data preparation that they should be excluded)
trlevels <- trlevels[!trlevels %in% c("pollinator", "pollinator.arthropod",
                                      "decomposer.acari", "decomposer.arthropod", "decomposer.myriapod", "decomposer.soilfungi")]
```
...
length(unique(herbivore[Trophic_level == "herbivore.arthropod" & Group_fine == "Hymenoptera", Plot]))
```
| Dataset    | Hemiptera | Coleoptera  | Hymenoptera | Orthoptera |
| --------   | --------- |------------ |------------ |----------- |
| no Plots   |  139      |     139     |  **145**    |  139       |
| no Species |  197      |     182     |    6        |  16        |

Special case for Hymenoptera species : 
```{r, eval = F}
aggregate(Plot ~ Species, herbivore[Group_fine == "Hymenoptera", ], length)
```
|            Species  | Plot |
|-------------------- |------|
| Ametastegia_tenera  | 139  |
|   Dulophanes_morio  | 139  |
|  Tenthredo_arcuata  | 113  |
| Tenthredo_mioceras  | 139  |
|    Tenthredo_notha  | 119  |
|   Tenthredo_zonula  | 139  |

*Tenthredo arcuata* and *Tenthredo notha* were measured on less than 139 plots (113 and 119). The remaining species were measured on 139 plots.
...
rm(testplotpresence)
```
The 2 species are removed. Reason : they were measured on a very different plot set than the other species. For the analysis, a complete dataset is required (as much overlap as possible). Loss of plots if they are included is inevitable (113 plots are present only, 127 plots could be used in previous analysis).
```{r}
herbivore <- herbivore[!Species %in% c("Tenthredo_arcuata", "Tenthredo_notha"), ]
```
updated species-plot table : 
...
```{r, eval = F}
#TODO leave this chunk? depends on plotNAset and therefore circular.. maybe better use arthro_plotset or completely delete
...
```{r}
herbivore <- herbivore[Trophic_level == "herbivore.arthropod", .(Species, Plot, value)]
nrow(herbivore) == 139 * 399 # 399 species in 139 plots present . If number is not correct, some species were not measured in all plots.
# remove from spdiv
...
```
```{r}
#TODO : maybe better just exclude them from the beginning from trlevels.
...
#TODO keep that? : trlevels <- trlevels[!trlevels == "secondary.consumer.myriapod"]
...
rm(birds) #; rm(bats); rm(batspecies)

# find plots with 0 species and remove them
test <- data.table::data.table(aggregate(value ~ Plot, tertiary.consumer, sum))
barplot(height = test$value)
removeplots <- test[value == 0, Plot] # 5 plots with 0 species
tertiary.consumer <- tertiary.consumer[!Plot %in% removeplots, ]
rm(removeplots)

nrow(tertiary.consumer) == 145*67 # expected number of rows present
```
check betadiversity
```{r,eval = F}
# check betadiversity
beta.tertiary.consumer <- prepare_for_betapair(tertiary.consumer)
beta.tertiary.consumer <- betapart::beta.pair(beta.tertiary.consumer, index.family = "sorensen")
...
test[tertiary.consumer == 0, ] # 23 such cases.
...
# note : no need to add back NA plots, as they are excluded from this analyis anyways
...
nrow(protist.eukaryvore) == 149 * 217 # TRUE : The expected number of rows is present.
...
```{r}
soilfungi.decomposer <- spdiv[Trophic_level == "decomposer.soilfungi"]
...
length(intersect(unique(soilfungi.decomposer[Year == "2014", Species]), unique(soilfungi.decomposer[Year == "2017", Species])))
```
...
nrow(soilfungi.decomposer) == 150 * 2332 # TRUE : all expected rows are present
...
```{r}
soilfungi.pathotroph <- spdiv[Trophic_level == "pathotroph.soilfungi"]
...
nrow(soilfungi.pathotroph) == 150 * 285 # TRUE : all expected rows present
...
2011, 2014, 2017
```{r}
soilfungi.symbiont <- spdiv[Trophic_level == "symbiont.soilfungi"]
testplotpresence <- unique(soilfungi.symbiont[, .(Year, Plot)])
visdat::vis_miss(data.table::dcast(testplotpresence, Plot ~Year))
rm(testplotpresence)
```
several missing plots in all years
```{r, eval=F}
...
nrow(soilfungi.symbiont) == 142 * 385 # all expected combinations present
...
| #species |     40     |     64    |      6    |
| #Plots   |    139     |    141*   |    150    |
*it becomes clear later, that arthropods in 2019 have 9 plots with 0 species --> need to be removed.
```{r, eval = F}
...
#TODO können ev. decomposer wieder benutzt werden? Wenn, dann only 2019.
...
```{r}
plant.pathogen <- spdiv[Trophic_level == "plantpathogen.fungi"]
```
```{r}
bp_overv <- data.table(aggregate(value ~ Plot, plant.pathogen, sum))
barplot(height = bp_overv$value)
abline(h = mean(bp_overv$value), col = "red")
abline(h = median(bp_overv$value), col = "green")
# 2 plots without species are removed:
plant.pathogen <- plant.pathogen[!Plot %in% bp_overv[value == 0, Plot], ]
```
84 species in 142 plots
```{r}
...
length(unique(plant.pathogen$Plot))
nrow(plant.pathogen) == 84 * 142
...
```{r, eval = F}
#TODO move that step below, to impute betadiverstiy data?
...
nrow(tr[which(tr[,3] != 1000), ]) ==  (150*150) - ((150*150 / 2) +150/2) # 11175 rows left
# masterbeta <- tr[!is.na(tr[, 3]),] #TODO changed that : no removal of NA values, but of values = 1000
masterbeta <- tr
