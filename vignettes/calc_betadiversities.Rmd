...
# 
# a <- rbind(tests, testt)
# a[, plotyear := paste(trlevel_year, Plot)]
# # unique(a$plotyear)[1]
# b <- data.table::dcast.data.table(a[plotyear == unique(a$plotyear[1])], Year ~ Species)
# b[is.na(b)] <- 0
# beta <- betapart::beta.pair(b[,-1], index.family = "sorensen")
# 
# 
# b <- data.table::dcast.data.table(a, plotyear ~ Species)
# 
# beta <- betapart::beta.pair(b[,-1], index.family = "sorensen")
# 
# # 1 Plot, many years
# m <- cor(test[, .(Species, value, Plot)])
```
