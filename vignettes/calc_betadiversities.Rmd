...
## Tertiary consumer
only included birds, not bats. Bats are not expected to have direct effects on any of the included functions.
```{r, eval = T}
#TODO HERE STECKEN GEBLIEBEN HIER WEITERMACHEN
# wurden birds und bats jetzt benutzt oder nicht? (update text oben!)
...
# note : following block is uncommented to exclude bats from analysis
...
#------------------- considerations about bird plots
#TODO wait for new bird data and set plots without species to betadiversity == 0
...
beta.belowground.herbivore <- prepare_for_betapair(tertiary.consumer)
beta.belowground.herbivore <- betapart::beta.pair(beta.belowground.herbivore, index.family = "sorensen")
test <- beta.belowground.herbivore$beta.sor
test <- as.matrix(test)
...
p <- data.table::data.table(aggregate(value ~ Plot, bats, sum))


# remove plots with 0 species
tertiary.consumer <- tertiary.consumer[!Plot %in% test[value == 0, Plot], ]
#------------------- considerations about bird plots
```
