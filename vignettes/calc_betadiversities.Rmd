...
herbivore.arthropod <- herbivore[Plot %in% arthro_plotset & Trophic_level == "herbivore.arthropod" | Trophic_level == "herbivore.snail", ]
rm(arthro_plotset); rm(herbivore)
```
...
decomposer.soilfungi <- data.table::setDT(soilfungi.decomposer)[data.table::CJ(Species = Species, Plot = Plot, unique = T), on=.(Species, Plot)]
decomposer.soilfungi[is.na(value), value := 0 ]
nrow(decomposer.soilfungi) == 150 * 2332 # TRUE : all expected rows are present
spdiv <- spdiv[!Trophic_level == "decomposer.soilfungi"]
rm(soilfungi.decomposer)
```
...
pathotroph.soilfungi <- data.table::setDT(soilfungi.pathotroph)[data.table::CJ(Species=Species,Plot=Plot,unique=T), on=.(Species, Plot)]
pathotroph.soilfungi[is.na(value), value := 0 ]
nrow(pathotroph.soilfungi) == 150 * 285 # TRUE : all expected rows present
spdiv <- spdiv[!Trophic_level == "pathotroph.soilfungi"]
rm(soilfungi.pathotroph)
```
...
Only 2011 is chosen.
```{r}
symbiont.soilfungi <- soilfungi.symbiont[Year == "2011", .(Species, Plot, value)]
# adding back missing plot-species combinations (code from Caterina)
symbiont.soilfungi <- data.table::setDT(symbiont.soilfungi)[data.table::CJ(Species=Species,Plot=Plot,unique=T), on=.(Species, Plot)]
symbiont.soilfungi[is.na(value), value := 0 ]
nrow(symbiont.soilfungi) == 142 * 385 # all expected combinations present
spdiv <- spdiv[!Trophic_level == "symbiont.soilfungi"]
rm(soilfungi.symbiont)
```
...
```
```{r}
bp_overv <- data.table::data.table(aggregate(value ~ Plot, plant.pathogen, sum))
barplot(height = bp_overv$value)
abline(h = mean(bp_overv$value), col = "red")
abline(h = median(bp_overv$value), col = "green")
...
plantpathogen.fungi <- data.table::setDT(plant.pathogen)[data.table::CJ(Species=Species,Plot=Plot,unique=T), on=.(Species, Plot)]
plantpathogen.fungi[is.na(value), value := 0 ]

length(unique(plantpathogen.fungi$Plot))
nrow(plantpathogen.fungi) == 84 * 142
...
#TODO HERE HIER STEcken geblieben: inspect the chosen plot set
# altes plot set : 127 plots
