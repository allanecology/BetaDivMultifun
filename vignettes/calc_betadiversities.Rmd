...
#TODO what did I mean with the comment below?
...
**herbivore.arthropod** consist of 4 groups ("Hemiptera"   "Coleoptera"  "Hymenoptera" "Orthoptera"), some contain missing plots. Chose the plot intersect with complete coverage.
```{r}
...
length(unique(herbivore[Trophic_level == "herbivore.arthropod" & Group_fine == "Orthoptera", Plot]))
```
...
```{r, eval=F}
length(unique(herbivore[Trophic_level == "herbivore.arthropod", Species])) # 401
...
nrow(herbivore) == 139 * 401 # 401 species in 139 plots present . If number is not correct, some species were not measured in all plots.
#TODO : error HERE stecken geblieben HERE HIER: 
nrow(herbivore[Group_fine == "Hymenoptera", ]) == 139*6 # too few values
aggregate(Plot ~ Species, herbivore[Group_fine == "Hymenoptera", ], length)
# again Tethredo_arcuata and Tenthredo notha have some missing plots!
# From report : exclude these 2 species? The trophic level herbivores included two species (\gls{tenthredo arcuata} and \gls{tenthredo notha}) which contained missing entries in many plots. Therefore they were excluded from analysis. 
# check out code here : https://github.com/biodiversity-exploratories-synthesis/Grassland_diversity_dataset/blob/master/201116_UpdateGRLdataset.R
# caterina solved the problem somehow.
