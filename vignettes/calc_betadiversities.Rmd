...
```{r, eval=T}
#TODO : if script is still working, remove those - are included in DESCRIPTION file of the package
require(naniar)
require(ggplot2)
require(grid)
require(VennDiagram)
...
#TODO : this is a test. remove row above if I find problems
...
test <- data.table(aggregate(value ~ Species, autotroph, sum))
...
rm(testplotpresence)
```
only run if `plotNAset` is already known.
```{r, eval = F}
# are the same plots missing in all years?
...
```{r, eval = F}
# only run if the plot set is known already
