...
decomposer.soilfungi <- add_back_missing_plots_species_combinations(decomposer.soilfungi)
...
pathotroph.soilfungi <- add_back_missing_plots_species_combinations(pathotroph.soilfungi)
...
length(setdiff(unique(soilfungi.symbiont[Year == "2011", Plot]), unique(soilfungi.symbiont[Year == "2014", Plot])))
...
if combined additively (plot present in 1 year is enough : 149 plots)

alternative : years 11 and 14 chosen (common set of species : 141, 244 species are different)
if soilfungi is present in one year, it is assumed to be present in all years
```{r}
symbiont.soilfungi <- unique(soilfungi.symbiont[Year %in% c("2011", "2014"), .(Species, Plot, value)])
```
...
# altes plot set : 127 plots, neues : 114
...
[1] "decomposer.soilfungi" "12342"                "species"             
...
```{r}
#TODO : add function to replace the missing plot-species combinations of 0 with corresponding betadiversities!
# gan maybe find out with NaN values.
for(t in trlevels){
  tr <- get(t)
