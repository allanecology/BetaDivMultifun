...
exclude <- c(exclude, "pollinator.arthropod", "omnivore.snail", "omnivore.ant", "omnivore.arthropod", "decomposer.arthropod", "decomposer.acari", "decomposer.myriapod", "carnivore.acari", "herbivore.snail", "protist.bacterivore")
...
hist(plants$value)
```
Although working with a range of 6 years, still many plant species are not present in many plots. This indicates that the assumption of stable autotroph species was correct.

```{r}
# remove lichens and plants, not used any more
rm(plants) ; rm(lichens); rm(lichenspecies); rm(autotrophnew); rm(test)
...
# note that currently, herbivore.snail is excluded.
```
...
length(unique(herbivore[Group_broad == "Hemiptera", Species]))
...
# length(unique(herbivore$Species)) # 681 species
...
# summary : 681 species in 150 plots
rm(herbivore, temp)
```
...
```{r}
#TODO HERE in sept 2023 : go through this code with updated diversity dataset.
#   potentially add new years, and make sure things still run.
...
```
Dataset IDs : 16871 16894 16893 16897 16895 20146
Years : 2008, 2011
Plots : 139
Species : 174
Groups : "Coleoptera" "Araneae"    "Hemiptera"  "Neuroptera" "Orthoptera" "Myriapoda" 
...
## protist.bacterivore TODO REMOVED
2011, 2017 datasets
```{r}
...
symbiont.soilfungi <- add_back_missing_plots_species_combinations(symbiont.soilfungi, all_plots) #57094 missing combinations were added.
